...
- `POST /v1/estimate-parses` creates a workflow assessment.
- `GET /v1/estimate-parses` lists recent assessment summaries.
- `GET /v1/estimate-parses/{request_id}` retrieves a stored assessment.
...
docker run --rm -p 8000:8000 --env-file .env collision-estimate-parser-api
```
...
ruff check .
ruff format --check .
docker build -t collision-estimate-parser-api:ci .
```
